...
$id = $_GET['id'];
$b = mysqli_fetch_array(mysqli_query($conect, "SELECT * FROM tb_artikel WHERE id_artikel='$id'"));
...
if (isset($_POST['input'])) {
    ...
    if (empty($foto)) {
        ...
        $qus = mysqli_query($conect, "UPDATE `tb_artikel` SET `judul`='$judul',`isi`='$isi' WHERE id_artikel='$id'");
        if ($qus) {
            echo "<script>alert('input berhasil');document.location='../artikel.php'</script>";
        } else {
            echo "<script>alert('input gagal silahkan coba lagi');</script>";
        }
    } else {
        unlink('../../../public/img/gambar_artikel/' . $gambarbaru);
        move_uploaded_file($foto, '../../../public/img/gambar_artikel/' . $newnama);

        $sql = mysqli_query($conect, "UPDATE `tb_artikel` SET `judul`='$judul',`isi`='$isi',`gambar`='$newnama' WHERE id_artikel='$id'");
        if ($sql) {
            echo "<script>alert('input berhasil');document.location='../artikel.php'</script>";
        } else {
            echo "<script>alert('input gagal silahkan coba lagi');</script>";
