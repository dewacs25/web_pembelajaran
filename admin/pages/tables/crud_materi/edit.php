...
$id = $_GET['id'];
$b = mysqli_fetch_array(mysqli_query($conect, "SELECT * FROM tb_materi WHERE id_materi='$id'"));
...
if (isset($_POST['input'])) {
    ...
    if (empty($foto)) {
        ...
        $qus = mysqli_query($conect, "UPDATE `tb_materi` SET `judul_materi`='$judul_materi',`isi`='$isi' WHERE id_materi='$id'");
        if ($qus) {
        ...
        unlink('../../../public/img/gambar_materi/' . $gambarbaru);
        move_uploaded_file($foto, '../../../public/img/gambar_materi/' . $newnama);

        $sql = mysqli_query($conect, "UPDATE `tb_materi` SET `judul_materi`='$judul_materi',`isi`='$isi',`gambar`='$newnama' WHERE id_materi='$id'");
        if ($sql) {
            echo "<script>alert('input berhasil');document.location='../materi.php'</script>";
        } else {
            echo "<script>alert('input gagal silahkan coba lagi');</script>";
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form action="" method="POST" autocomplete="off" enctype="multipart/form-data">
                                    ...
                                    <input type="text" class="form-control" name="judul_materi" placeholder="Masukan Judul" value="<?= $b['judul_materi']; ?>" size="50" required autocomplete="off">
                                    <br><br>
